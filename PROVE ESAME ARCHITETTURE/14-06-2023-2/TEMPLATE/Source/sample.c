...
int DIFF_MAX_MIN(int VETT[], unsigned int dim);
...
int main (void) {
  ...
  //LPC_SC -> PCONP |= (1 << 21);  // TURN ON TIMER 1
	...
	*/
	...
	init_timer(3, 0, 0, 2, 0x1312D0); //MR0  calcolo K/2, dopo il quale scatena l'evento, ho I attivato per entrare nell'handler
	init_timer(0, 0, 0, 3, 0x3F940A); //MR1	calcolo K=T*Fr=(1/60Hz)*25*10^6, dopo ogni K scatena l'evento e resetta il timer (R attivato), ho I attivato per entrare nell'handler
