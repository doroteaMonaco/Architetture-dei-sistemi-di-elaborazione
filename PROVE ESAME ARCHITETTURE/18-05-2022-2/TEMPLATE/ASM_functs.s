...
;				EOR R4, R4, R9
;				
;		mov r2, #0xFFFFFFFF ;max
				;mov r3, #0x7FFFFFFF ;min		
;				
...
;    mov   r1, #0        @ r1 conterrà il conteggio
;naive_loop:
;    cmp   r0, #0        @ Se r0 è zero, abbiamo finito
;    beq   naive_done
;    and   r2, r0, #1    @ r2 = r0 & 1 (estrae il bit meno significativo)
;    add   r1, r1, r2    @ aggiunge il bit estratto al conteggio
;    lsr   r0, r0, #1    @ rshift logico: r0 = r0 >> 1
;    b     naive_loop
...
;per vedere se è primo faccio la udiv, poi la mul e calcolo il resto, se è zero è primo
