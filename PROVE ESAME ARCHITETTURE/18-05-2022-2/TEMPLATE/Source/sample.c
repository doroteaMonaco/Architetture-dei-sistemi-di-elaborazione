...
int totale_pressioni_con_filtro(unsigned char VETT[], unsigned int n, unsigned char max, unsigned char min);
...
int main (void) {
  ...
  //LPC_SC -> PCONP |= (1 << 23);  // TURN ON TIMER 3	
	...
	*/
	...
	init_timer(1, 0, 0, 3, 0.1*0xBEBC20); //MR1	calcolo K=T*Fr=(1/60Hz)*25*10^6, dopo ogni K scatena l'evento e resetta il timer (R attivato), ho I attivato per entrare nell'handler
	init_timer(2, 0, 0, 3, 0.1*0x23C3460);
