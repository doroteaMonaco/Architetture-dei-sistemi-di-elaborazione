...
unsigned short int PUNTI_HOME = 0;
unsigned short int PUNTI_GUEST = 0;
unsigned short int SECONDI = 0;
char stat = 0;
char maxAdv = 0;
char compute_statistics(int HOME[N], int GUEST[N], char *maxAdv);
...
int main (void) {
  ...
  BUTTON_init();												/* BUTTON Initialization              */
	...
	*/
	...
	init_timer(2, 0, 0, 3, 0.01*0xB2D05E00); //MR0  calcolo K/2, dopo il quale scatena l'evento, ho I attivato per entrare nell'handler
	init_timer(1, 0, 0, 3, 0x2625A0); //MR1	calcolo K=T*Fr=(1/60Hz)*25*10^6, dopo ogni K scatena l'evento e resetta il timer (R attivato), ho I attivato per entrare nell'handler
	init_timer(0, 0, 0, 3, 0x4C4B40);
	
	LPC_SC -> PCONP |= (1 << 21);
	LPC_SC -> PCONP |= (1 << 22);  // TURN ON TIMER 2
	LPC_SC -> PCONP |= (1 << 23);
	
	enable_timer(2); //Attivo timer
