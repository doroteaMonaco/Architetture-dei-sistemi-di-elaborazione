...
{					
	...
	if ((LPC_GPIO1->FIOPIN & (1 << 29)) == 0 && (LPC_GPIO1->FIOPIN & (1 << 28)) != 0 && (LPC_GPIO1->FIOPIN & (1 << 27)) != 0 && (LPC_GPIO1->FIOPIN & (1 << 26)) != 0 && (LPC_GPIO1->FIOPIN & (1 << 25)) != 0 && monitor == 1) { // JOYSTICK UP
				// AZIONE UP
		up++;	
		if(valoreIniziale != 0 && valoreIniziale != 1){
			valoreIniziale = 1;
		}			
		else if(valoreCorrente != 0 && valoreCorrente != 1){
				if(valoreCorrente == 0){
					disable_timer(3);
					VAL = LPC_TIM3->TC;
					enable_timer(3);
					valoreCorrente = 1;
					tempiVariazioni[index] = 0x17D7840-VAL;
					index++;
					if(index == N){
						dutyCicle = duty_cycle(valoreIniziale, 0x17D7840, tempiVariazioni, index);
						disable_timer(3);
						reset_timer(3);
						LED_Out(index);
						LED_On(7);
