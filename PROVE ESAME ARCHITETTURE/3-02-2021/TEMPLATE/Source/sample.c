...
int monitor = 0; //0->MONITOR, 1->ACQUISIZIONE
unsigned char valoreIniziale = -1, valoreCorrente = -1;
int index = 0;
unsigned int VAL = 0;
...
unsigned int tempiVariazioni[N];
...
int main (void) {
  ...
  BUTTON_init();												/* BUTTON Initialization              */
	...
	*/
	...
	init_timer(3, 0, 0, 5, 0x17D7840); //MR0  calcolo K/2, dopo il quale scatena l'evento, ho I attivato per entrare nell'handler
	init_timer(0, 0, 0, 3, 0x2FAF080); //MR1	calcolo K=T*Fr=(1/60Hz)*25*10^6, dopo ogni K scatena l'evento e resetta il timer (R attivato), ho I attivato per entrare nell'handler
