...
int concat_sum(unsigned short int VETT[N], unsigned int dim, char *alarm);
void svuotaVettore();
...
int main (void) {
  ...
  BUTTON_init();												/* BUTTON Initialization              */
	...
	*/
	...
	init_timer(3, 0, 0, 3, 0.1*0xEE6B280); //10s//MR0  calcolo K/2, dopo il quale scatena l'evento, ho I attivato per entrare nell'handler
	init_timer(0, 0, 0, 3, 0x7270E0); //300ms per pressione //MR1	calcolo K=T*Fr=(1/60Hz)*25*10^6, dopo ogni K scatena l'evento e resetta il timer (R attivato), ho I attivato per entrare nell'handler
	init_timer(1, 0, 0, 3, 0xBEBC20);
	enable_timer(3); //Attivo timer
