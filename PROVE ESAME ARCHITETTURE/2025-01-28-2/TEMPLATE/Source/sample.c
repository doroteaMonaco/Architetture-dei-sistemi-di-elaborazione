...
void svuotaVettore();
int compute_sum_and_ranges(unsigned short int VETT[], int index, char lowerB, char upperB, int *rc);
...
int main (void) {
  ...
  LPC_SC -> PCONP |= (1 << 23);  // TURN ON TIMER 3	
	...
	*/
	...
	init_timer(2, 0, 0, 2, 0.1*0x3D090); //MR0  calcolo K/2, dopo il quale scatena l'evento, ho I attivato per entrare nell'handler
	init_timer(0, 0, 0, 3, 0x7F2815); //MR1	calcolo K=T*Fr=(1/60Hz)*25*10^6, dopo ogni K scatena l'evento e resetta il timer (R attivato), ho I attivato per entrare nell'handler
	init_timer(1, 0, 0, 3, 0.1*0x17D7840);
	init_timer(3, 0, 0, 3, 0.1*0x4C4B40);
