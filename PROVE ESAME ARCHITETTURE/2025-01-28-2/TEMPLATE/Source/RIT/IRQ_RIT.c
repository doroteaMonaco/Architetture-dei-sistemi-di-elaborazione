...
{					
	static int up=0, left=0, down=0, right=0, select=0, up_left = 0, up_right = 0, down_left, down_right; //1->UP, 2->RIGHT, 3->LEFT, 4->DOWN	
...
//			reset_RIT(); //da COMMENTARE se gestisco anche joystick
			//INT0=20 e EINT0_IRQn, KEY1==22 e EINT1_IRQn, KEY2=24 e EINT2_IRQn
			if(count1 > 2){
				somma = compute_sum_and_ranges(VETT, index, lowerB, upperB, &rc);
				disable = 1;
				LED_Out(somma^0xFF);
				enable_timer(3);
			}
...
//			reset_RIT(); //da COMMENTARE se gestisco anche joystick
	...
	//reset_RIT(); //abilito solo se si blocca
