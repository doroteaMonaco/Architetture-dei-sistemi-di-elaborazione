...
**
******************************************************************************/
int first=1, index=0, disabilitato=0, discard=0, complete=0;
unsigned char VAL;
unsigned char ExtractBits(int v);
unsigned char check_fibonacci(unsigned char* V, unsigned char n, unsigned char m, unsigned char v, unsigned char b);
void RIT_IRQHandler (void)
{					
	static int up=0, left=0, down=0, right=0, select=0; //1->UP, 2->RIGHT, 3->LEFT, 4->DOWN	
	static int debounce0=0, debounce1=0, debounce2=0, value;	
			...
			switch(debounce1){
				case 1: /*dopo INTERVALLO RIT (es.50ms) faccio azione*/
					...
					if(first==1) {
						...
						if(index==0 && VAL%2==0) {
							if(check_fibonacci(VETT,index,M,VAL,BOUNDARY)==1) {
								LED_Out(0xFF);
								index++;
								enable_timer(0);
								disabilitato=1;
