...
dutycycle		PROC
				...
				MOV R4, #0 ;iterazione pari o dispari
				MOV R5, #0; tempo totale a 1
				MOV R11, #100
...
loop0			LDR R6, [R2] 
				CMP R4, #1 ;nelle iterazioni dispari ho i tempi in cui è a 1
				ADDEQ R5, R5, R6
				ADDEQ R4, R4, #1
				SUBNE R4, R4, #1
...
loop1			LDR R6, [R2] 
				CMP R4, #0 ;nelle iterazioni pari ho i tempi in cui è a 1
				ADDEQ R5, R5, R6
				ADDEQ R4, R4, #1
				SUBNE R4, R4, #1
