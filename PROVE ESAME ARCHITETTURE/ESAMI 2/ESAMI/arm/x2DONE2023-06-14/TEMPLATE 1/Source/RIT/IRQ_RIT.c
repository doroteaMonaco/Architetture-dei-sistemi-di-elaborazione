...
int DIFF_MAX_MIN(int *b, unsigned int c);
unsigned char extractbyte(int a);

int d;
void RIT_IRQHandler (void)
{					
	static int position=-1; //1->UP, 2->RIGHT, 3->LEFT, 4->DOWN	
	static int debounce0=0, debounce1=0;	
	static int diff;
	...
	if((LPC_PINCON->PINSEL4 & (1 << 22)) == 0){		/*BOTTONE PREMUTO*/
		...
		if((LPC_PINCON->PINSEL4 & (1 << 20)) == 0){		/*BOTTONE PREMUTO*/
			...
			if((LPC_GPIO2->FIOPIN & (1<<10)) == 0){ 	
				...
				switch(debounce0){
					case 1: /*dopo INTERVALLO RIT (es.50ms) faccio azione*/
						diff=DIFF_MAX_MIN(buffer,i);
						d=diff;
						display=extractbyte(diff);
