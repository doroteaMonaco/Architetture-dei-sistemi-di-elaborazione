...
**
******************************************************************************/
unsigned int res, calcola_somma_prodotto(unsigned int* V, unsigned int n, char* a) ;
char alarm;
int disabilitato=0, first=1;
void RIT_IRQHandler (void)
{					
	static int up=0, left=0, down=0, right=0, select=0; //1->UP, 2->RIGHT, 3->LEFT, 4->DOWN	
	static int debounce0=0, debounce1=0, debounce2=0, value, led_value, i;	
	...
	if((LPC_PINCON->PINSEL4 & (1 << 20)) == 0){ /*BOTTONE PREMUTO*/
		...
		if((LPC_GPIO2->FIOPIN & (1<<11)) == 0){ 	
		...
		else if(disabilitato==0 && first==0) {	/* button released */
			debounce1=0;			
			//disable_RIT(); //da COMMENTARE se gestisco anche joystick (nel rit non entro solo premendo il joystick, infatti enable_RIT() va messo anche nel main
			//reset_RIT(); //da COMMENTARE se gestisco anche joystick
			//INT0=20 e EINT0_IRQn, KEY1==22 e EINT1_IRQn, KEY2=24 e EINT2_IRQn
			...
			else if(counter0==1 && counter1==0) {
				VAR1+=3;
				
			}
			else {
				res=calcola_somma_prodotto(VETT,index,&alarm);
				disabilitato=1;
				counter2=0;
				disable_timer(2);
				reset_timer(2);
		...
		else if(disabilitato==0 && first==1) {
			NVIC_EnableIRQ(EINT1_IRQn);							 /* RIATTIVO interruzione*/
			debounce1=0;
	...
	if ((LPC_GPIO1->FIOPIN & (1 << 26)) == 0 && disabilitato==0) { // JOYSTICK DOWN
				// AZIONE DOWN
		down++;
		VETT[index]=VAR1;
		index++;
