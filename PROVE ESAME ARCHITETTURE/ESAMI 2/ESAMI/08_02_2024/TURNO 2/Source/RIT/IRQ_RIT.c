...
**
******************************************************************************/
unsigned int avg_vett(unsigned int* V, unsigned int dim, char* flag);
unsigned char extractbyteneg(unsigned int v);
unsigned int av;
char f;
			...
			switch(debounce0){
				case 1: /*dopo INTERVALLO RIT (es.50ms) faccio azione*/
					...
					if(index==N) {
						...
						counter0=0;
						clear_vett();
						av=avg_vett(VETT,index,&f);
						if(f==0) {
							dis=extractbyteneg(av);
							LED_Out(dis);
