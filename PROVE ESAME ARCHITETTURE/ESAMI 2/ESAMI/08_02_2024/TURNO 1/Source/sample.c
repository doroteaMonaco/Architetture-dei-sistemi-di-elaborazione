...
//NON � visibile
#define N 6
uint32_t cs=0, vett[N];
unsigned char extractbyte(uint32_t VAR);
unsigned char extractopposite(unsigned char VAR);
int i=0;
unsigned char contatore_inrange(uint32_t *VETT1, const unsigned int N1);
void clear_vett();
...
int main (void) {
  ...
  BUTTON_init();												/* BUTTON Initialization              */
	...
	*/
	clear_vett();
	init_timer(0, 0, 0, 0x3, 0x3D090); //CICLICO, 0.01s*25000000 alla premuta di KEY1
	init_timer(1, 0, 0, 0x3, 0x2DC6C0); //CICLICO, 0.12s*25000000
	init_timer(2, 0, 0, 0x3, 0x5B8D80); //CICLICO, 5s*25000000 7735940 
	init_timer(3, 0, 0, 0x5, 0x2FAF080); //SI, 6s*25000000 8F0D180 
