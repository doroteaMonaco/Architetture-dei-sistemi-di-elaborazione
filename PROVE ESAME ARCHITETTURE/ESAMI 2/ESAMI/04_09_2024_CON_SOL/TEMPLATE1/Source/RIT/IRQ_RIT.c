...
unsigned int av;
char flag;
unsigned int avg_vett(unsigned int* V, unsigned int dim, char* f);
unsigned char ExtractBits(unsigned int a);
		...
		if((LPC_GPIO2->FIOPIN & (1<<10)) == 0){ 	
			...
			switch(debounce0){
				case 1: /*dopo INTERVALLO RIT (es.50ms) faccio azione*/
					...
					else if(VAR!=VETT[index-1]) {
						VETT[index]=VAR;
						index++;
					}
					
					if(index==N) {
						av=avg_vett(VETT,N,&flag);
						clear_vett();
						disable_timer(0);
						reset_timer(0);
						counter0=0;
						if(flag==0) {
							display=ExtractBits(av);
							LED_Out(display);
