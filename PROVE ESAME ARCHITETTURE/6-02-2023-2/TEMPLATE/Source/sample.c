...
int differenza_media_positivi_negativi(int VETT[N], unsigned int num, char *over);
...
int main (void) {
  ...
  BUTTON_init();												/* BUTTON Initialization              */
	...
	*/
	...
	init_timer(0, 0, 0, 5, 0xFD51DA80); //MR0  calcolo K/2, dopo il quale scatena l'evento, ho I attivato per entrare nell'handler
	init_timer(1, 0, 0, 3, 0x7DE290); //MR1	calcolo K=T*Fr=(1/60Hz)*25*10^6, dopo ogni K scatena l'evento e resetta il timer (R attivato), ho I attivato per entrare nell'handler
	//enable_timer(0); //Attivo timer
	init_timer(2, 0, 0, 3, 0xBEBC20);
	enable_timer(0);
