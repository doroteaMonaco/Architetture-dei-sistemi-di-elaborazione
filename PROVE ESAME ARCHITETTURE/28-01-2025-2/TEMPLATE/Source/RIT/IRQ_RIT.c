...
**
******************************************************************************/
volatile int disableINT = 0, disableKEY1 = 0, disableKEY2 = 0, disable = 0;
int somma = 0;
int value = 0;

void RIT_IRQHandler (void)
{					
	static int up=0, left=0, down=0, right=0, select=0, up_left = 0, up_right = 0, down_left = 0, down_right = 0; //1->UP, 2->RIGHT, 3->LEFT, 4->DOWN	
...
//enable_timer();
	...
	if((LPC_PINCON->PINSEL4 & (1 << 20)) == 0 && disableINT == 0){ /*BOTTONE PREMUTO*/
		debounce0++;
		...
		if((LPC_PINCON->PINSEL4 & (1 << 22)) == 0 && disableKEY1 == 0){ /*BOTTONE PREMUTO*/
		debounce1++;
		if((LPC_GPIO2->FIOPIN & (1<<11)) == 0){ 	
			...
			if(count1 >= 0.1*2){
				...
				disable_timer(1);
				reset_timer(1);
				somma = compute_sum_and_ranges(VETT, N, lowB, uppB, &rc);
				disable = 1;
				enable_timer(3);
			}
	...
	if((LPC_PINCON->PINSEL4 & (1 << 24)) == 0 && disableKEY2 == 0){ /*BOTTONE PREMUTO*/
		debounce2++;
	...
	if ((LPC_GPIO1->FIOPIN & (1 << 28)) == 0 && (LPC_GPIO1->FIOPIN & (1 << 29)) != 0 && (LPC_GPIO1->FIOPIN & (1 << 27)) != 0 && (LPC_GPIO1->FIOPIN & (1 << 26)) != 0 && (LPC_GPIO1->FIOPIN & (1 << 25)) != 0 && disable == 0) { // JOYSTICK RIGHT
				// AZIONE RIGHT
		right++;
		LED_Out(0x0);
		count0 = 1;
	...
	if ((LPC_GPIO1->FIOPIN & (1 << 27)) == 0 && (LPC_GPIO1->FIOPIN & (1 << 28)) != 0 && (LPC_GPIO1->FIOPIN & (1 << 29)) != 0 && (LPC_GPIO1->FIOPIN & (1 << 26)) != 0 && (LPC_GPIO1->FIOPIN & (1 << 25)) != 0 && disable == 0) { // JOYSTICK LEFT
				// AZIONE LEFT
	...
	if((LPC_GPIO1->FIOPIN & (1 << 25)) == 0 && (LPC_GPIO1->FIOPIN & (1 << 28)) != 0 && (LPC_GPIO1->FIOPIN & (1 << 27)) != 0 && (LPC_GPIO1->FIOPIN & (1 << 26)) != 0 && (LPC_GPIO1->FIOPIN & (1 << 29)) != 0 && disable == 0){ // SELECT (R3,L3)
			// AZIONE SELECT
		select++;
	}
	else {
	...
	if ((LPC_GPIO1->FIOPIN & (1 << 26)) == 0 && (LPC_GPIO1->FIOPIN & (1 << 28)) != 0 && (LPC_GPIO1->FIOPIN & (1 << 27)) == 0 && (LPC_GPIO1->FIOPIN & (1 << 29)) != 0 && (LPC_GPIO1->FIOPIN & (1 << 25)) != 0 && disable == 0) { // JOYSTICK DOWN
		...
		if(down_left!=0) { //azione solo al rilascio di down e non sempre quando non lo premo
			VAR = 6;
			VETT[index] = VAR;
			index++;
			if(index == N){
	...
	if ((LPC_GPIO1->FIOPIN & (1 << 26)) == 0 && (LPC_GPIO1->FIOPIN & (1 << 28)) == 0 && (LPC_GPIO1->FIOPIN & (1 << 27)) != 0 && (LPC_GPIO1->FIOPIN & (1 << 29)) != 0 && (LPC_GPIO1->FIOPIN & (1 << 25)) != 0 && disable == 0) { // JOYSTICK DOWN
		...
		if(down_right!=0) { //azione solo al rilascio di down e non sempre quando non lo premo
			VAR = 7;
			VETT[index] = VAR;
			index++;
			if(index == N){
