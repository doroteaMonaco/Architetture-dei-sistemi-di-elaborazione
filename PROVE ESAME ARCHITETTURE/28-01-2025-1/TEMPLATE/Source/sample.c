...
unsigned short int SEQ[N] = {7, 2, 9, 6, 11, 1, 9, 5, 7, 4};
int index1 = 0;
int fine = 0;
char SCORE = 0;
char CLASSIFICA[M];
...
char updateRank(char SCORE, char CLASSIFICA[M], char *max_diff, char *PODIO);
int led;
void leggiVettore(unsigned short int SEQ[], int n);
int main (void) {
  ...
  BUTTON_init();												/* BUTTON Initialization              */
	...
	LED_On(led);
	index1++;
	init_timer(0, 0, 0, 7, 0.1*SEQ[index1]*25000000);
	enable_timer(0);
	...
	*/
	...
	init_timer(1, 0, 0, 3, 0xBEBC20);//2Hz //MR0  calcolo K/2, dopo il quale scatena l'evento, ho I attivato per entrare nell'handler
  init_timer(2, 0, 0, 3, 0x2625A0); ////10HzMR1	calcolo K=T*Fr=(1/60Hz)*25*10^6, dopo ogni K scatena l'evento e resetta il timer (R attivato), ho I attivato per entrare nell'handler
...
void leggiVettore(unsigned short int SEQ[], int n){
		...
		for(i=0; i<N-1; i+=2){
			led = SEQ[i];
			LED_On(led);
			init_timer(0, 0, 0, 3, 0.1*SEQ[i+1]*25000000);
			enable_timer(0);
		}
		if(i == N){
			fine = 1;
