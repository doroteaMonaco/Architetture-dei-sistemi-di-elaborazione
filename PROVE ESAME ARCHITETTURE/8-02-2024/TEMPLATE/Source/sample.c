...
unsigned char VETT[N] = {35, 20, 80, 46};  
...
void svuotaVettore();
unsigned char analisi_accuratezza(unsigned char VETT[], unsigned char VETT2[], unsigned int n, char RES[]);
...
int main (void) {
  ...
  BUTTON_init();												/* BUTTON Initialization              */
	...
	*/
	...
	index++;
	init_timer(1, 0, 0, 3, 0x2625A0); //MR0  calcolo K/2, dopo il quale scatena l'evento, ho I attivato per entrare nell'handler
	init_timer(2, 0, 0, 3, 0x5F5E10); //MR1	calcolo K=T*Fr=(1/60Hz)*25*10^6, dopo ogni K scatena l'evento e resetta il timer (R attivato), ho I attivato per entrare nell'handler
